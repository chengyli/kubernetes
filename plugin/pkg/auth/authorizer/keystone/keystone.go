...
func (ka *keystoneAuthorizer) Authorize(a authorizer.Attributes) error {
	glog.V(4).Infof("Authorizing user %s for namespace %s", a.GetUserName(), a.GetNamespace())

	if a.IsReadOnly() && a.GetNamespace() == "" {
		return nil
	}
	...
	hasRole, err := ka.osClient.roleCheck(ka.userMap[a.GetUserName()], ka.osClient.getDefaultTenant())
	if hasRole && err != nil {
		return nil
	}

	if a.GetNamespace() == "default" || a.GetNamespace() == "" {
		hasRole, err = ka.osClient.roleCheck(ka.userMap[a.GetUserName()], ka.osClient.getDefaultTenant())
	} else {
	...
	if err != nil {
		glog.V(2).Infof("Keystone authorization failed: %v", err)
...
func (osClient *OpenstackClient) roleCheck(userID string, tenantID string) (bool, error) {
	glog.V(2).Infof("Running role check on userid %s for tenantID %s", userID, tenantID)
	if userID == "" {
		return false, errors.New("UserID null during authorization")
	}
	if tenantID == "" {
		return false, errors.New("TenantID null during authorization")
	}
	hasRole := false
...
func (osClient *OpenstackClient) getDefaultTenant() string {
	return osClient.config.TenantId
}
