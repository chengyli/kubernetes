...
func main() {
	...
	for t := time.Now(); time.Since(t) < 5*time.Minute; time.Sleep(10 * time.Second) {
		...
		glog.Infof("Waiting for elasticsearch-logging service to become available in namespace '%s'", namespace)
	}
	// If we did not find an elasticsearch logging service then log a warning
	// and return without adding any unicast hosts.
	if elasticsearch == nil {
		glog.Warningf("Failed to find the elasticsearch-logging service in namespace '%s': %v", namespace, err)
		return
	} else {
		glog.Infof("Found elasticsearch-logging service in namespace '%s'", namespace)
	}
	...
	count := 0
	for t := time.Now(); time.Since(t) < 5*time.Minute; time.Sleep(10 * time.Second) {
		glog.Info("Fetching endpoints for elasticsearch-logging service.")
