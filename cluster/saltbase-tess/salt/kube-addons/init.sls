...
/etc/kubernetes/addons/cluster-monitoring/prometheus:
  file.recurse:
    - source: salt://kube-addons/cluster-monitoring/prometheus
    - include_pat: E@(^.+\.yaml$|^.+\.json$)
    - template: jinja
    - user: root
    - group: root
...
/etc/kubernetes/addons/cluster-monitoring/grafana:
  file.recurse:
    - source: salt://kube-addons/cluster-monitoring/grafana
    - include_pat: E@(^.+\.yaml$)
    - template: jinja
    - user: root
    - group: root
...
/etc/kubernetes/addons/cluster-monitoring/assertions:
   file.recurse:
    - source: salt://kube-addons/cluster-monitoring/assertions
    - include_pat: E@(^.+\.yaml$)
    - template: jinja
    - user: root
    - group: root
...
/etc/kubernetes/addons/logstash-elasticsearch:
  file.recurse:
    - source: salt://kube-addons/logstash-elasticsearch
    - include_pat: E@^.+\.yaml$
    - template: jinja
    - user: root
    - group: root
...
{{ pillar.get('systemd_system_path') }}/kube-addons.service:
  file.managed:
    ...
    - context:
        ...
        KUBERNETES_MASTER_NAME: {{ pillar.get('api_servers_with_port')}}
  cmd.wait:
