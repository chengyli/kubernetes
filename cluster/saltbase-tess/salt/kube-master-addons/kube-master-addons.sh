...
while true; do
 if which docker 1>/dev/null 2>&1; then
   if docker load -i /etc/kubernetes/kube-bins/kube-apiserver.tar 1>/dev/null 2>&1; then
     let loadedImageFlags="$loadedImageFlags|1";
   fi;
   if docker load -i /etc/kubernetes/kube-bins/kube-scheduler.tar 1>/dev/null 2>&1; then
     let loadedImageFlags="$loadedImageFlags|2";
   fi;
   if docker load -i /etc/kubernetes/kube-bins/kube-controller-manager.tar 1>/dev/null 2>&1; then
